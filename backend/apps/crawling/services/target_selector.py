...
def get_due_targets(limit: int = 3, target_type: str = "search"):
    ...
    # 1) 아직 한 번도 안 돌린 대상
    never_crawled_qs = all_targets.filter(last_crawled_at__isnull=True).order_by(
        "-priority", "created_at"
    )

    selected_ids = list(never_crawled_qs.values_list("id", flat=True)[:limit])
    ...
    if remaining > 0:
        ...
        candidates = all_targets.filter(last_crawled_at__isnull=False).order_by(
            "last_crawled_at", "-priority", "created_at"
        )
    ...
    selected_targets = CrawlTarget.objects.filter(id__in=selected_ids).order_by(
        "last_crawled_at", "-priority", "created_at"
    )
